...
'*****************************************************************
'* 软件名称：GB/BIG5/UTF-8 文件编码批量转换程序
'*
'* 版权信息：此软件由 阿勇(fxy_2002@163.com) 开发，请保留此信息！！
'*      作者开放此源码，仅是出于学习交流之目的，请勿用于商业用途。
'*
'* 作者主页：pc-soft.cn    e-mail：fxy_2002@163.com
'*
'* 程序功能：将 GB、BIG5、UTF-8 文件相互转换，方便的批量处理能力，
'*      主要用于网站文件编码方式的整体转换上。
'*
'* 特别提醒：程序不保证文件绝对安全，使用前请备份！
'*
'* 开发环境：Visual Basic6.0(SP6) 简体中文、Windows XP(SP2) 简体中文
'*      在其它环境下未做测试，因此程序不一定能正常运行。
'*
'* 已知缺陷：对于 BIG5 编码的文件，若执行 UTF-8 转换，保存到 UTF-8 文
'*      件中的是伪编码，实际上是 GBK 繁体字的 UTF-8 编码，而非 BIG5。
'*      这么做的原因在于在简体 Windows 系统中，我还没有找到好的办法对
'*      BIG5 进行 UTF-8 编码，只能对 GBK 进行 UTF-8 转换，使之看起来
'*      象一个繁体字文件。
'*
'* 最后修改：2006/05/09
'*
...
'@ 版本历史：
'@
'@      1.21 -- 当前版本，生成 UTF-8 时可选是否带BOM(标示自身编码
'@              方式和字节序的特殊字串，BYTE ORDER MARK）
'@
'@      1.2 -- 添加在转换过程中进行文件内容添加、替换功能；
'@             该功能使本软件也能作为一个文本替换软件使用
'@
...
';
Sub Main()
    If GetSetting("GB2UTF8", "Parameter", "FirstRunMe", 0) = 0 Then
        showAbout   '第一次运行，显示关于窗口
        SaveSetting "GB2UTF8", "Parameter", "FirstRunMe", 1
    End If
...
'--------------功能函数------------------------------
';
Sub showAbout(Optional ByRef hWnd As Long)
'显示软件信息
    MsgBoxA "GB/BIG5/UTF-8 文件编码批量转换程序。" & vbCrLf & vbCrLf _
         & "功    能：将 GB/BIG5/UTF-8 编码的文件相互转换，" & vbCrLf _
         & "          主要应用于网站编码方式的整体转换上。" & vbCrLf & vbCrLf _
         & "软件版本：1.21" & vbCrLf _
         & "编译日期：2006/05/09 " & vbCrLf & vbCrLf _
         & "程序作者：阿勇(fxy_2002@163.com)" & vbCrLf _
         & "作者主页：WWW.PC-SOFT.CN" & vbCrLf & vbCrLf _
         & "特别提醒：程序不保证文件绝对安全，使用前请备份！" & vbCrLf & vbCrLf _
         & "免费软件，欢迎使用！", _
         "OK，知道了", IIf(hWnd = 0, "GB/BIG5/UTF-8 文件编码转换", "关于..."), vbOKOnly + vbInformation, hWnd
End Sub
